...
import { METHODS, TOPICS, LANGUAGES } from "@/lib/catalog";
...
export default async function PsychologistDetailPage({
  ...
  const p = await prisma.psychologistProfile.findUnique({
    ...
    include: {
      ...
      availability: true,
      reviews: {
        where: { isHidden: false, isModerated: true },
  ...
  if (!p || p.verification !== "VERIFIED") return notFound();
      ...
      <div className="grid gap-5 md:grid-cols-3 md:gap-6">
        <div className="md:col-span-2 space-y-5 md:space-y-6">
          <div className="card">
            ...
            <p className="mt-4 whitespace-pre-line text-slate-700">
              {p.bio || "Краткое описание появится позже."}
            </p>
            {p.approach && (
              <>
        ...
        <aside className="space-y-4">
          <div className="card">
            ...
            <AvailabilityCalendar psychologistId={p.id} sessionMinutes={p.sessionMinutes} />
          </div>
